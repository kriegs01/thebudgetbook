...
const Accounts: React.FC<AccountsProps> = ({ accounts, onAdd, onDelete, onEdit, onDeactivate, onReactivate, loading = false, error = null }) => {
  ...
  // Calculate the effective status for an account, taking deactivation_date scheduling into account.
  // If status is already 'inactive', return 'inactive'. Otherwise check if deactivation_date has passed.
  const calculateEffectiveStatus = (acc: Account): 'active' | 'inactive' => {
    if (acc.status === 'inactive') return 'inactive';
    if (!acc.deactivationDate) return 'active';
    const now = new Date();
    const currentYear = now.getFullYear();
    const currentMonth = now.getMonth(); // 0-indexed
    const deactYear = parseInt(acc.deactivationDate.year);
    const deactMonth = monthNames.findIndex(m => m.toLowerCase() === acc.deactivationDate!.month.toLowerCase()); // 0-indexed
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Preserve the existing status and deactivationDate only when editing
      const originalAccount = isEditing ? accounts.find(a => a.id === editingId) : null;
      const created: Account = {
        ...
        status: isEditing ? (originalAccount?.status ?? 'active') : 'active',
        deactivationDate: isEditing ? originalAccount?.deactivationDate : undefined,
      };
  ...
  const renderAccount = (acc: Account, isInactive = false) => {
    ...
    return (
      <div key={acc.id} className={`bg-white p-6 rounded-2xl shadow-sm border transition-all relative group overflow-hidden ${isInactive ? 'border-gray-200 opacity-75' : 'border-gray-100 hover:border-indigo-200'}`}>
        ...
        <div className="flex justify-between items-start mb-6">
          <div className={`p-3 rounded-xl ${isInactive ? 'bg-gray-100 text-gray-400' : isCredit ? 'bg-purple-50 text-purple-600' : 'bg-green-50 text-green-600'}`}>
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between">
            ...
            {isInactive ? (
              <span className="text-[10px] font-bold px-2 py-0.5 rounded uppercase bg-gray-100 text-gray-500 flex items-center gap-1">
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className={`text-2xl font-bold ${isInactive ? 'text-gray-400' : 'text-gray-900'}`}>{formatCurrency(acc.balance)}</p>
          </div>
        </div>
  ...
  const activeAccounts = accounts.filter(a => calculateEffectiveStatus(a) === 'active');
  const inactiveAccounts = accounts.filter(a => calculateEffectiveStatus(a) === 'inactive');
  const debitAccounts = activeAccounts.filter(a => a.type === 'Debit');
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-500" onClick={() => setOpenMenuId(null)}>
      ...
      {/* Inactive Accounts Section — collapsed by default */}
      {inactiveAccounts.length > 0 && (
        <section>
          <button
            onClick={() => setIsInactiveOpen(!isInactiveOpen)}
            className="flex items-center space-x-2 mb-4 text-gray-500 hover:text-gray-700 font-bold text-lg"
          >
            {isInactiveOpen ? <ChevronDown className="w-5 h-5" /> : <ChevronRight className="w-5 h-5" />}
            <PowerOff className="w-5 h-5 text-gray-400" />
            <span className="text-base font-bold text-gray-600 uppercase tracking-widest">Inactive Accounts ({inactiveAccounts.length})</span>
          </button>
          {isInactiveOpen && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {inactiveAccounts.map(acc => renderAccount(acc, true))}
            </div>
          )}
        </section>
      )}
