...
import { Plus, Trash2, Calendar, WalletCards, CalendarDays } from 'lucide-react';
...
export const SandboxView: React.FC<SandboxViewProps> = ({ onClose, liveIncomeTxs, liveSpendTxs, activeSetup, allSetups, currentYear }) => {
  ...
  // 🟢 NEW: Dynamic Date Range Controls
  const [forecastMonths, setForecastMonths] = useState<number>(3); // Defaults to 3 months out
  ...
  // Generate End Date Options (Up to 12 months ahead)
  ...
  const timeline = Array.from({ length: totalPeriods }).map((_, i) => {
    ...
    // Plot Mock Purchases onto this specific period
    ...
    mockPurchases.forEach(purchase => {
      ...
      if (purchase.type === 'one-off') {
        ...
        const monthlyAmount = purchase.amount / duration;
  ...
  return (
    <div className="animate-in slide-in-from-bottom-4 duration-500 bg-[#F4F3EF] dark:bg-gray-950 min-h-screen pb-32 w-full p-4 md:p-8 relative overflow-hidden flex flex-col">
      ...
      <div className="flex justify-between items-center mb-6 border-b-4 border-black pb-4 shrink-0">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 font-bold text-sm">Time-Based Forecasting</p>
        </div>
        
        <button 
          onClick={onClose}
          className="px-4 py-2 bg-red-100 text-red-700 border-4 border-black rounded-xl font-black uppercase tracking-wider text-xs shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all"
        >
          Exit
        </button>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 flex-1 min-h-0">
        ...
        {/* 🟡 FORECAST BOARD */}
        <div className="w-full lg:w-2/3 flex flex-col gap-6 order-1 lg:order-2 overflow-hidden">
          ...
          {/* TIMELINE UI */}
          <div className="p-6 bg-white dark:bg-gray-900 border-4 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex-1 overflow-y-auto lg:overflow-x-auto lg:overflow-y-hidden custom-scrollbar">
            <div className="mb-6 flex justify-between items-end shrink-0 sticky left-0">
              <h2 className="text-xl font-black uppercase">Timeline Forecast</h2>
            </div>

            <div className={`relative flex w-full flex-col space-y-6 lg:space-y-0 lg:flex-row lg:space-x-6 lg:min-w-max pb-4`}>
          ...
          <div className={`shrink-0 flex justify-between items-center p-6 border-4 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-colors duration-500 ${isAbsolutelySafe ? 'bg-green-400' : 'bg-red-400'}`}>
            <div>
              <span className="font-black text-black uppercase tracking-widest text-sm block mb-1">Sandbox Verdict</span>
              <span className="font-bold text-black/80 text-xs">
                {isAbsolutelySafe ? 'All periods stay above your safety net! 🎉' : 'Warning: A period dips below your safety net! ⚠️'}
              </span>
            </div>
            {safetyNet > 0 && (
               <div className="text-right">
                  <span className="text-[10px] font-black uppercase block text-black/60">Safety Net</span>
                  <span className="font-black text-xl text-black">₱{safetyNet.toLocaleString()}</span>
               </div>
            )}
          </div>
        </div>

        {/* 🎶 APPLE MUSIC TRAY / LEFT COLUMN */}
        <div className={`
          order-2 lg:order-1 w-full lg:w-1/3 flex flex-col gap-6 lg:sticky lg:top-4 lg:translate-y-0 lg:h-[calc(100vh-8rem)]
        ...
        `}>
          ...
          <div className="px-6 pb-12 pt-6 lg:p-0 max-h-[75vh] lg:h-full overflow-y-auto custom-scrollbar flex flex-col gap-6 bg-[#F4F3EF] lg:bg-transparent">
            ...
            <div className="p-6 bg-white dark:bg-gray-900 border-4 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] shrink-0">
              <h2 className="text-xl font-black mb-2 uppercase">Safety Net</h2>
            ...
            <div className="p-6 bg-white dark:bg-gray-900 border-4 border-black rounded-2xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex-1 flex flex-col">
              <div className="shrink-0 mb-6">
                <h2 className="text-xl font-black mb-4 uppercase">What if I spend...</h2>
                ...
                <div className="space-y-3 mb-4">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input type="number" value={newPurchaseAmount} onChange={(e) => setNewPurchaseAmount(e.target.value)} placeholder={purchaseType === 'installment' ? 'Total Amount' : 'Amount'} className="w-full pl-6 bg-gray-50 dark:bg-gray-800 border-2 border-black rounded-xl px-3 py-2 outline-none font-black text-xs" />
                    </div>
              ...
              <div className="space-y-3 overflow-y-auto flex-1 pr-1 custom-scrollbar min-h-[150px] border-t-2 border-dashed border-gray-200 pt-4">
                {mockPurchases.length === 0 ? (
                  ...
                  mockPurchases.map(purchase => (
                    <div key={purchase.id} className="flex justify-between items-center bg-gray-50 dark:bg-gray-800 border-2 border-black p-3 rounded-xl">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <span className="text-sm font-black text-red-500">₱{purchase.amount.toLocaleString()}</span>
