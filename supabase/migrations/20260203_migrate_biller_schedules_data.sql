...
BEGIN
  ...
  FOR biller_record IN SELECT id, schedules FROM billers WHERE schedules IS NOT NULL LOOP
    ...
    FOR schedule_record IN SELECT * FROM jsonb_array_elements(biller_record.schedules) LOOP
      -- Insert into payment_schedules table if it doesn't already exist
      ...
      SELECT
        ...
        END,
        biller_record.id
      ON CONFLICT (biller_id, month, year) DO NOTHING; -- Skip if already exists
    END LOOP;
  END LOOP;
  
  RAISE NOTICE 'Successfully migrated biller schedules to payment_schedules table';
END $$;
