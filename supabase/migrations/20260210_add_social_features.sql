...
ALTER TABLE transactions ADD COLUMN IF NOT EXISTS friend_user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL;
ALTER TABLE installments ADD COLUMN IF NOT EXISTS friend_user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL;

-- (Optional: Upgrade test environment tables as well)
ALTER TABLE transactions_test ADD COLUMN IF NOT EXISTS friend_user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL;
ALTER TABLE installments_test ADD COLUMN IF NOT EXISTS friend_user_id UUID REFERENCES auth.users(id) ON DELETE SET NULL;
